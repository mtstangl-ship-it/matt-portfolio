...
function hidePaths(svg: SVGSVGElement): void {
  svg.querySelectorAll('g[class*="road-"] path').forEach((node) => {
    ...
    const len = path.style.getPropertyValue("--path-len") || path.getTotalLength();
...
export function EYFootprint({ tabActive }: { tabActive: boolean }) {
  ...
  const goToView = useCallback(
    async (next: View) => {
      ...
      const enteringDetail = view === "georgia" && next !== "georgia";
      ...
      const applyNext = async () => {
        ...
        const cardDelay = reduceMotion ? 80 : enteringDetail || swappingCity ? CARD_DELAY_MS : 120;
    ...
    >
      <div className="impact-ey-footprint__stage">
        ...
        {showChrome && (
          <button
            type="button"
            className="impact-ey-footprint__zoomout"
        ...
        {showChrome && breadcrumbActive && (
          <nav className="impact-ey-footprint__breadcrumb" aria-label="Footprint views">
            <span className="impact-ey-footprint__breadcrumb-pos">
              {breadcrumbActive.index}/4 · {breadcrumbActive.label}
            </span>
            <div className="impact-ey-footprint__breadcrumb-track" role="tablist">
              {BREADCRUMB.map((b) => {
                const lit =
                ...
                return (
                  <button
                    ...
                    className={`impact-ey-footprint__breadcrumb-dot${lit ? " is-lit" : ""}${view === b.view ? " is-active" : ""}`}
        ...
        {/* Card overlay */}
        <div
          className={`impact-ey-footprint__card${cardVisible ? " is-visible" : ""}`}
          aria-live="polite"
        >
          {isCityView(view) && (
            <>
