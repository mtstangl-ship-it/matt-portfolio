  ...
  return (
    <section className="border-b border-white/[0.06] px-4 py-12 sm:px-6 sm:py-16">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-2 max-w-2xl  text-[0.9375rem] text-dashboard-ink-muted">{evenOver.subtitle}</p>
        ...
        <div className="mt-12 border-t border-white/[0.07] pt-10">
          <h3 className=" text-[clamp(1.05rem,2vw,1.2rem)] font-semibold tracking-[-0.02em] text-white">
            {nonNegotiables.title}
          </h3>
          <p className="mt-2  text-[0.875rem] text-dashboard-ink-muted">{nonNegotiables.intro}</p>
          <ul className="mt-6 space-y-3">
            {nonNegotiables.lines.map((line) => (
              <li key={line} className="flex items-start gap-3  text-[0.8125rem] leading-relaxed text-dashboard-ink-light/88">
