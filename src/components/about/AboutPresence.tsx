  ...
  return (
    <section className="border-b border-white/[0.06] bg-dashboard-muted/20 px-4 py-12 sm:px-6 sm:py-16">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-3 max-w-2xl  text-[0.9375rem] leading-relaxed text-dashboard-ink-muted">{presence.intro}</p>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:gap-5">
          {presence.items.map((item) => (
            ...
            >
              <p className=" text-[0.8125rem] font-semibold text-accent-signal/95">{item.title}</p>
              <p className="mt-2  text-[0.8125rem] leading-relaxed text-dashboard-ink-muted">{item.body}</p>
            </div>
          ))}
        </div>
      </div>
