#!/usr/bin/env node
/**
 * Bake halftone into Wipro hero PNG (Tier-A parity with Case 01 baked PNG pipeline).
 *
 * Mirrors `scripts/process-ey-hero-halftone.mjs` — same Bayer pipeline, tuned to
 * the same canvas as `centaur-literal.png` so all Tier A heroes share identical
 * density / tonal range under the CSS finishing layer (`tier-a/hero-chrome.css`).
 *
 * Run from repo root:
 *   node scripts/process-wipro-hero-halftone.mjs
 */
...
const INPUT = path.join(ROOT, "public/images/case-wipro/elc-workshop.png");
const OUTPUT = path.join(ROOT, "public/case-studies/wipro-elc-workshop-halftone.png");
