#!/usr/bin/env node
/**
 * Bake halftone into EY hero PNG (Tier-A parity with Case 01 baked PNG pipeline).
 *
 * Phase 1 note: ImageMagick is not assumed installed; investigation used `sharp`
 * metadata plus luminance / micro-contrast heuristics instead of `identify -verbose`.
 *
 * Landed pipeline (tunable — Matt may request another pass):
 * - Cover resize to match centaur-literal.png canvas (1600×900).
 * - Grayscale → linear(0.52, 6) → gamma(1.4).
 * - 8×8 Bayer ordered dither, 8 gray levels, modulation strength ≈ step × 0.95.
 *
 * Run from repo root:
 *   node scripts/process-ey-hero-halftone.mjs
 */
...
const INPUT = path.join(ROOT, "public/images/case-ey/core-booth-hero.jpg");
const OUTPUT = path.join(ROOT, "public/case-studies/ey-core-halftone.png");
